...
-- 🔒 Kompatibles string.match für Lua 5.1 (Fallback über find/sub)
...
-- 📦 Fortschritt aus der Registry holen (ohne SavedVariables zu berühren)
...
-- 📌 KILLING_BLOW (Floating Combat Text) – generische Kills
  ...
  for _, a in ipairs(KAMN.achievements or {}) do
    if (a.type == "kill" or a.type == "generickill") and (a.active ~= false) and not a.complete then
      ...
      if KAMN and KAMN.debug then
        DEFAULT_CHAT_FRAME:AddMessage("[KAM Debug] Kill matched (KILLING_BLOW): " .. a.name .. " (" .. a.id .. ") = " .. newProgress)
      end
    end
  end
...
-- 📌 Eigener Kill-Blow über Hostile Death-Zeile („You have slain …“ / „Du hast … getötet!“)
-- 🔧 Änderung: Hier zählen jetzt auch 'generickill' (vorher nur kill/namedkill/bosskill)
function KAMN_HandleHostileDeath(event, arg1)
  ...
  -- Weiterleitung zu Proximity‑Kills, wenn es keine eigene „You have slain…“/„Du hast…“‑Zeile ist
  ...
  for i = 1, table.getn(KAMN.achievements or {}) do
    ...
    if (a.type == "kill" or a.type == "generickill" or a.type == "namedkill" or a.type == "bosskill")
       and (a.active ~= false) and not a.complete then
      local matched = false
      ...
      -- Generischer Kill (kein Zielname vorgegeben)
      ...
      if matched then
        ...
        updated = true
        if KAMN and KAMN.debug then
          DEFAULT_CHAT_FRAME:AddMessage("[KAM Debug] Kill matched (HostileDeath): " .. a.name .. " (" .. a.id .. ") = " .. newProgress)
        end
      end
    end
  end
...
-- 📌 Named-Kill‑Gruppen (Sammlung benannter Gegner)
function KAMN_HandleNamedKillGroup(event, msg)
  ...
  for i = 1, table.getn(KAMN.achievements or {}) do
    local a = KAMN.achievements[i]
    if a and a.type == "namedkillgroup" and a.groupMatch then
      for j = 1, table.getn(a.groupMatch) do
        ...
        if match ~= "" and (lname == match or string.find(lname, match)) then
          ...
          updated = true
          if KAMN and KAMN.debug then
            DEFAULT_CHAT_FRAME:AddMessage("[KAM Debug] NamedKillGroup: " .. name .. " → " .. a.name .. " (" .. a.id .. ") = " .. newProgress)
          end
          break
        end
      end
...
-- 📌 Gruppen-/Nähe‑Kills aus Systemzeilen („X has slain Y“, „Y dies.“ / „Y stirbt.“)
function KAMN_HandleProximityKill(event, msg)
  ...
  for i = 1, table.getn(KAMN.achievements or {}) do
    local a = KAMN.achievements[i]
    if a and (a.type == "namedkill" or a.type == "bosskill") and (a.active ~= false) and not a.complete then
      local matched = false
      ...
      if matched then
        ...
        updated = true
        if KAMN and KAMN.debug then
          DEFAULT_CHAT_FRAME:AddMessage("[KAM Debug] ProximityKill: " .. a.name .. " (" .. a.id .. ") = " .. newProgress)
        end
      end
    end
  end
